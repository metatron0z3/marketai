...
def get_trade_data(instrument_id, date_str=None):
    ...
    if date_str:
        # QuestDB doesn't support DATE() function, use timestamp range instead
        next_date = (pd.to_datetime(date_str) + pd.Timedelta(days=1)).strftime('%Y-%m-%d')
        date_filter = f"AND ts_event >= '{date_str}T00:00:00.000000Z' AND ts_event < '{next_date}T00:00:00.000000Z'"
    else:
        date_filter = ""
